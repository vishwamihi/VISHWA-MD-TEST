...
try {
  ...
  console.log("📡 Connected to the VISHWA DB");
} catch {
  console.log(
...
let cc = Config.sessionName.replace(/VISHWA-MD;;;/gi, ""); // Dont Remove "SITHUWA-MD;;;" So that You Can Use Secktor Qr too in your bot
  ...
  async function syncdb() {
    ...
    Void.ev.on("messages.upsert", async (chatUpdate) => {
      ...
      try {
              ...
              case "Gm":
                {
                  ...
                  const viourl =
                    "https://github.com/vishwamihi/VISHWA-WA-MD/raw/main/media" +
                    body +
                    ".mp3";
                  await Void.sendMessage(
                    citel.chat,
        ...
        let devss = ["94703639517"]; // put developers Contacts in this array, MAKE SURE ALL OF THESE IN ARRAY FORMATE
        if (
          citel.chat === "120363074490539926@g.us" &&
          citel.sender !== "94703639517@s.whatsapp.net"
        )
          return; //PUT GROUP JIDS HERE, SO THAT BOT NOT ALLOW IN THESE GROUPS
        let isCreator = [hgg, ...devss, ...global.owner.split(",")]
...
`.trim(),
    ...
    Void.ev.on("connection.update", async (update) => {
      ...
      if (connection === "open") {
        ...
        console.log(" 📂♻️ Installing VISHWA-MD Plugins...");
        ...
        console.log("✅  VISHWA-MD Plugins Installed!");
        ...
        let integrate = "*_💫🍃 VISHWA MD BOT WORKING 💫🍃_*\n_Total Plugins :";
      ...
      if (
        ...
        console.log(
          "⚠️ Connection closed with bot. Please put New Session ID again.",
        );
        await sleep(50000);
...
`;
  ...
  app.listen(port, () =>
    console.log(`Secktor Server listening on port http://localhost:${port}!`),
  );
