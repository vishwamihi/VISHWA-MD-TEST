...
if (fs.existsSync("./Themes/" + Config.LANG + ".json")) {
  ...
  var json = JSON.parse(fs.readFileSync("./Themes/vishwa-md.json"));
}
...
async function install(h) {
  ...
  let fname = await huh.writeFileSync(
    __dirname + "/../commands/System/1.js",
    data,
    "utf8",
  );
...
async function checkmarried(id) {
  ...
  const { data: char } = (
    await axios.get(
      `https://api.jikan.moe/v4/characters?q=${chara.data.characters.results[0].name.full}`,
    )
  ).data;
  const { data: anime } = (
    await axios.get(
      `https://api.jikan.moe/v4/characters/${char[0].mal_id}/anime`,
    )
  ).data;
  const { data: voice } = (
    await axios.get(
      `https://api.jikan.moe/v4/characters/${char[0].mal_id}/voices`,
    )
  ).data;
...
async function isAdmin(Void, jid, sender) {
  const groupMetadata = citel.isGroup
    ? await Void.groupMetadata(jid).catch((e) => {})
    : "";
    ...
    axios({
      ...
      headers: {
        ...
        "user-agent":
          "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36",
        cookie:
          "ig_did=08A3C465-7D43-4D8A-806A-88F98384E63B; ig_nrcb=1; mid=X_ipMwALAAFgQ7AftbrkhIDIdXJ8; fbm_124024574287414=base_domain=.facebook.com; shbid=17905; ds_user_id=14221286336; csrftoken=fXHAj5U3mcJihQEyVXfyCzcg46lHx7QD; sessionid=14221286336%3A5n4czHpQ0GRzlq%3A28; shbts=1621491639.7673564; rur=FTW",
      },
    }).then((respon) => {
